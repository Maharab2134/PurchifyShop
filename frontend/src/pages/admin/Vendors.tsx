...
export default function AdminVendors() {
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      const params: { limit: number; status?: 'pending' | 'approved' } = { limit: 200 }
  ...
  const onAdd = form.handleSubmit(async (data) => {
    ...
    try {
      ...
      setAddOpen(false)
      form.reset({ name: '', email: '', address: '', whatsappNumber: '', contactName: '' })
      await load()
    } catch (e: unknown) {
  ...
  return (
      ...
      <><div className="p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <h1 className="text-xl sm:text-2xl font-semibold text-gray-800">Vendors</h1>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              type="button"
              onClick={() => { setAddOpen(true); form.reset({ name: '', email: '', address: '', whatsappNumber: '', contactName: '' }); }}
        ...
        {!loading && !error && vendors.length === 0 && (
          <div className="text-center py-12 bg-gray-50 rounded-xl">
            ...
            <p className="text-gray-600 mb-4">No vendors yet. Add a vendor to get started.</p>
            <Button
              type="button"
              onClick={() => { setAddOpen(true); form.reset({ name: '', email: '', address: '', whatsappNumber: '', contactName: '' }); }}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50">
                    ...
                    <th className="text-left py-3 px-4 font-medium text-gray-600">WhatsApp</th>
                ...
                <tbody>
                  {vendors.map((v) => (
                    <tr key={v.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4 text-gray-600">{v.whatsappNumber ?? '—'}</td>
                      <td className="py-3 px-4">
                        <span className={`px-2 py-0.5 rounded text-xs font-medium ${(v.status ?? 'pending') === 'approved' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'}`}>
                          {(v.status ?? 'pending') === 'approved' ? 'Approved' : 'Pending'}
                        </span>
                      ...
                      <td className="py-3 px-4 text-gray-600 max-w-[200px] truncate" title={v.address ?? undefined}>{v.address ?? '—'}</td>
        ...
        {addOpen && (
          <Modal open={true} onClose={() => setAddOpen(false)} title="Add Vendor" size="lg">
            <form onSubmit={onAdd} className="space-y-4">
              <div>
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <div>
                      <input {...field} className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 ${fieldState.error ? 'border-red-300' : 'border-gray-300'}`} placeholder="Vendor name" />
              ...
              <div>
                ...
                <Controller name="email" control={form.control} render={({ field }) => (
                  <input {...field} type="email" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="vendor@example.com" />
                )} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Contact Name</label>
                <Controller name="contactName" control={form.control} render={({ field }) => (
                  <input {...field} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="Contact person name" />
                )} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">WhatsApp Number</label>
                <Controller name="whatsappNumber" control={form.control} render={({ field }) => (
                  <input {...field} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="+880..." />
                )} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Address</label>
                <Controller name="address" control={form.control} render={({ field }) => (
                  <textarea {...field} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="Full address" />
                )} />
              </div>
        ...
        {editing && (
          <Modal open={true} onClose={() => setEditing(null)} title={`Edit Vendor: ${editing.name}`} size="lg">
