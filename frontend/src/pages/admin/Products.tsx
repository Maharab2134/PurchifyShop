...
export default function AdminProducts() {
  ...
  const load = async (page = 1, size = 10) => {
    ...
    try {
      const [pRes, cRes, vendorsRes] = await Promise.all([
        ...
        adminApi.vendors.list({ limit: 100, status: 'approved' }).catch(() => ({ data: { data: { vendors: [] } } })),
      ])
      setProducts(pRes.data.data?.products ?? [])
      setTotalProducts(pRes.data.data?.totalResults ?? pRes.data.data?.products?.length ?? 0)
      setCategories(cRes.data.data ?? [])
      setVendors(vendorsRes.data.data?.vendors?.map((v) => ({ id: v.id, name: v.name })) ?? [])
    } catch (e: unknown) {
  ...
  const onAdd = handleSubmit(
    async (data) => {
      ...
      try {
        ...
        showToast('Product created. Add variants on the Product page.', 'success')
        reset({
          name: '',
          shortDescription: '',
          description: '',
        ...
        let errorMsg = err.response?.data?.message ?? 'Failed to create product'
  ...
  const startRecord = totalProducts === 0 ? 0 : (currentPage - 1) * pageSize + 1
      ...
      <div className="p-4 sm:p-6">
        ...
        {!loading && !error && products.length === 0 && (
          <div className="text-center py-12 bg-gray-50 rounded-xl">
            ...
            <p className="text-gray-600 mb-4">No products yet. Add a product to get started.</p>
            <Button
              type="button"
              onClick={() => navigate('/dashboard/products/add')}
                    ...
                    <tr key={p.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4">
                        <img
                          ...
                          onError={(e) => {
                            e.currentTarget.src = generateProductPlaceholder(p.name, 80)
                          }}
                        />
                      </td>
                      <td className="py-3 px-4">
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1 mt-0.5">
                            {p.isFeatured && <span className="px-1.5 py-0.5 bg-indigo-100 text-indigo-700 rounded text-xs">Featured</span>}
                            {p.isNew && <span className="px-1.5 py-0.5 bg-green-100 text-green-700 rounded text-xs">New</span>}
                            {p.discountBadge && <span className="px-1.5 py-0.5 bg-red-100 text-red-700 rounded text-xs">{p.discountBadge}</span>}
                            {p.isOutOfStock && <span className="px-1.5 py-0.5 bg-gray-200 text-gray-600 rounded text-xs">Out of Stock</span>}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-baseline gap-1">
                          {p.isDiscountActive && p.originalPrice > p.discountedPrice && (
                            <span className="text-gray-400 line-through text-xs">৳{p.originalPrice.toFixed(2)}</span>
                          )}
                          <span className="font-medium text-gray-900">৳{p.discountedPrice.toFixed(2)}</span>
                        </div>
                      </td>
                      <td className="py-3 px-4 text-gray-600">{p.totalStock}</td>
                      <td className="py-3 px-4">
                        <span className={`px-2 py-1 text-xs font-medium rounded ${p.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600'}`}>
                          {p.status}
                        </span>
                      </td>
                ...
                <div className="flex gap-0.5 flex-wrap justify-center">
                  {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((item, index) => (
                      <div key={index}>
                        {item === '...' ? (
                          <span className="px-1 py-1 text-gray-600 text-xs">...</span>
                        ) : (
                          <button
        ...
        <ConfirmModal
          ...
          message={deleteProduct ? `Delete "${deleteProduct.name}"? This cannot be undone.` : ''}
          confirmLabel="Delete"
          danger
          loading={submitting}
          onConfirm={onDelete}
        ...
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          <div>
            ...
            <Controller name="shortDescription" control={control} render={({ field }) => (
              <input {...field} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="Brief summary — shown in product info (optional)" maxLength={500} />
            )} />
            <p className="text-xs text-gray-500 mt-1">Shown in product info / Description area. Max 500 chars.</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Description (long)</label>
            <RichTextEditor
              name="description"
              control={control}
              editorKey="add"
            ...
            />
            <p className="text-xs text-gray-500 mt-1">Shown in the Description section on product page. Styles (bold, lists, links) will display for users.</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Select vendor for this product. Used for order sharing via WhatsApp.</p>
          </div>
          ...
          <Controller name="images" control={control} render={({ field }) => (
            <div>
              <ImageUpload value={field.value} onChange={field.onChange} label="Product Images" maxFiles={6} />
              <p className="text-xs text-gray-500 mt-1">Add variants after saving the product.</p>
            </div>
          )} />
          <div className="flex gap-4">
            <label className="flex items-center gap-2">
              <Controller name="isNew" control={control} render={({ field }) => (
                <input type="checkbox" checked={!!field.value} onChange={(e) => field.onChange(e.target.checked)} onBlur={field.onBlur} ref={field.ref} name={field.name} className="rounded" />
              )} />
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
            <button type="button" onClick={onCancel} className="px-6 py-2.5 text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 rounded-lg font-medium">Cancel</button>
            <button type="submit" disabled={submitting} className="px-6 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium disabled:opacity-50 flex items-center gap-2">
              {submitting ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving...</> : <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>Save Product</>}
            </button>
          </div>
          {Object.keys(formErrors).length > 0 && (
            <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
              <p className="text-sm font-medium text-red-800 mb-1">Please fix:</p>
              <ul className="text-xs text-red-700 list-disc list-inside">{formErrors.name && <li>{formErrors.name.message}</li>}</ul>
            </div>
          )}
        </form>
    </Modal>
