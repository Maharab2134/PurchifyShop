...
export default function UserMenu({ menuOpen, closeMenu, user }: UserMenuProps) {
  ...
  const menuItems = [
    {
      routes: [
        { href: '/', label: 'Home', icon: <Home size={18} className="text-indigo-500" />, show: !isSuperAdmin },
        { href: '/orders', label: 'My Orders', icon: <ShoppingCart size={18} className="text-emerald-500" />, show: !isSuperAdmin },
        { href: '/profile', label: 'Profile', icon: <User size={18} className="text-blue-500" />, show: true },
        { href: '/contact-support', label: 'Contact Support', icon: <Group size={18} className="text-blue-500" />, show: !isSuperAdmin },
      ],
    },
    {
      routes: [
