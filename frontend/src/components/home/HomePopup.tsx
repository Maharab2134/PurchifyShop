      ...
      .then((data) => {
        ...
        if (popupSettings) {
          ...
          const shouldShowOnPage =
            pages.length === 0 || pages.includes(location.pathname);
          ...
          if (popupSettings.isActive && !hasShown) {
            ...
            if (!popupShown) {
              ...
              const checkWelcomeComplete = () => {
                const welcomeActive =
                  animSettings?.welcomeAnimation?.isActive ?? false;
                const delay =
                  welcomeActive && !welcomeCompleted
                    ? animSettings.welcomeAnimation.duration +
  ...
  const handleButtonClick = () => {
    if (settings?.buttonLink) {
      if (
        ...
        settings.buttonLink.startsWith("https://")
      ) {
